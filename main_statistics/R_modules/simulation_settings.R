...
parse_integer_vector_arg <- function(x, default = NULL) {
  ...
  out <- as.integer(round(vals))
  if (any(abs(vals - out) > 1e-10)) stop("Expected integer values, got: ", as.character(x[1L]))
  unique(out)
}
...
power_delta_grid_default <- function(scenario, dgp_type, include_zero = TRUE) {
  ...
  base_vals <- if (scenario %in% c("level_shift", "smooth_change", "sinusoidal_change")) {
    if (dgp_type == "BB") {
      c(0.050, 0.065, 0.080, 0.095, 0.110, 0.125)
    } else {
      c(0.005, 0.009, 0.013, 0.017, 0.021, 0.025, 0.029)
    }
  } else if (dgp_type == "BB") {
