# ==============================================================
# run_null_streamingcurve_simulation.R
# ============================================================== 
...
# ----------------------- user controls -----------------------
settings <- resolve_null_run_settings(args)
...
OUTPUT_DIR <- file.path(ROOT, "outputs", "null_raw")
...
summary_lines <- c(
  strrep("=", 78L),
  "Streaming-curve null-size simulation",
  ...
  sprintf("gamma values: %s", paste(settings$gamma_vec, collapse = ", ")),
  sprintf("CvM weights: %s", paste(settings$cvm_weights, collapse = ", ")),
...
if (!is.null(preflight$alt_warning)) {
  summary_lines <- append(summary_lines, sprintf("Benchmark detector status: %s", preflight$alt_warning), after = 11L)
}
if (!isTRUE(preflight$worker_bootstrap_ok) && core_plan$used > 1L) {
  summary_lines <- append(summary_lines, "Parallel status: PSOCK unavailable during preflight, using serial fallback.", after = 12L)
}
write_run_summary(summary_lines, OUTPUT_DIR, filename = "run_summary_null.txt")
...
param_grid <- expand.grid(m = settings$m_vals, T_value = settings$T_grid, dgp_type = settings$dgp_types, stringsAsFactors = FALSE)
for (i in seq_len(nrow(param_grid))) {
  combo <- param_grid[i, , drop = FALSE]
  file_tag <- sanitize_tag(sprintf("null_%s_m%s_T%s_nsim%s", combo$dgp_type, combo$m, canonical_T_scalar(combo$T_value), settings$nsim))
  ...
  message(sprintf("Running null simulation: DGP=%s | m=%d | T=%s | nsim=%d | workers=%d",
                  sim_par$dgp_type, sim_par$m, canonical_T_scalar(sim_par$T_value), settings$nsim, ncores))
...
message("Null-size simulation complete. Raw files are in: ", normalize_path2(OUTPUT_DIR, mustWork = FALSE))
