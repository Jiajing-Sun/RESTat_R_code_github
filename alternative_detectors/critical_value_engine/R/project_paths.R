...
find_project_root <- function(start = NULL,
                              markers = c("run_generate_alt_detector_cv.R", file.path("R", "project_paths.R"))) {
  ...
  stop(
    ...
    "or pass --root=/path/to/alt_detector_cv_codes."
  )
}

resolve_project_root <- function(default_start = NULL) {
  args <- parse_named_args()
  env_root <- Sys.getenv("ALT_DETECTOR_CV_ROOT", unset = "")
