# ==============================================================
# check_multiscale_tuning_targeted.R
# Representative rerun for multiscale MOSUM tuning sensitivity
# ==============================================================
...
normalize_scale_weight_name <- function(x) {
  ...
  if (w %in% c("INVSQRTH", "INV_SQRT_H", "1/SQRT_H", "SQRTH")) return("InvSqrtH")
  stop("Unknown multiscale weight. Supported: Equal, InvSqrtH")
}
...
multiscale_scale_names <- c("Equal", "InvSqrtH")
...
summarize_power_combo <- function(df, cv_emp) {
  ...
  out <- aggregate(
    cbind(raw_power = as.numeric(reject), sap = size_adjusted_reject, add = delay) ~
      scenario + dgp_type + T + s_star + delta + method_id + method_label + standardizer + hset_name + scale_weight_name,
    data = ok,
...
out_dir <- file.path(ROOT, "outputs", "tuning_diagnostics", "multiscale_targeted")
ensure_dir(out_dir)
...
write_csv_atomic(null_summary, file.path(out_dir, "null_summary.csv"), row.names = FALSE)
...
for (scenario_name in scenario_vec) {
  delta_vals <- power_delta_grid_default(scenario_name, "fMA1", include_zero = FALSE)
