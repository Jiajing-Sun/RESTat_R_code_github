...
tex_path <- file.path(root_dir, "simulation_section.tex")
...
ks_style_caption <- paste(
  "The legend below the final subfigure identifies the six method-boundary combinations."
)

std_style_caption <- paste(
  "The legend below the final subfigure identifies RSMS, SSMS, and HAC."
)
...
plot_theme <- function(base_size = 8.5) {
  theme_bw(base_size = base_size) +
    theme(
      ...
      panel.border = element_rect(fill = NA, colour = "#A9BDD0", linewidth = 0.30),
      panel.spacing.x = grid::unit(0.55, "lines"),
      strip.background = element_rect(fill = "#EAF4FF", colour = "#A9BDD0"),
      strip.text = element_text(size = base_size - 1, face = "bold", colour = "#2F5D86"),
...
ks_colors <- c(
  "RSMS, gamma=0" = method_colors[["RSMS"]],
  "RSMS, gamma=0.15" = method_colors[["RSMS"]],
  "SSMS, gamma=0" = method_colors[["SSMS"]],
  "SSMS, gamma=0.15" = method_colors[["SSMS"]],
  "HAC, gamma=0" = method_colors[["HAC"]],
  "HAC, gamma=0.15" = method_colors[["HAC"]]
)
ks_linetypes <- c(
  "RSMS, gamma=0" = "solid",
  "RSMS, gamma=0.15" = "dashed",
  "SSMS, gamma=0" = "solid",
  "SSMS, gamma=0.15" = "dashed",
  "HAC, gamma=0" = "solid",
  "HAC, gamma=0.15" = "dashed"
)
ks_shapes <- c(
  "RSMS, gamma=0" = 16,
  "RSMS, gamma=0.15" = 1,
...
std_colors <- method_colors
std_linetypes <- c(RSMS = "solid", SSMS = "solid", HAC = "solid")
...
nolegend_filename <- function(filename) {
  sub("\\.png$", "_nolegend.png", filename)
}

save_plot_with_nolegend <- function(p, filename, width, height, nolegend_height) {
  save_plot(p + theme(legend.position = "none"), nolegend_filename(filename), width, nolegend_height)
  save_plot(p, filename, width, height)
}

add_inset_legend_to_first_panel <- function(p, legend_nrow = 3L,
                                            x = 0.985, y = 0.04,
                                            width = 0.34, height = 0.28,
                                            just = c("right", "bottom")) {
  p_with_legend <- p +
    guides(
      colour = guide_legend(nrow = legend_nrow, byrow = TRUE),
    ...
    theme(
      ...
      legend.title = element_blank(),
      legend.background = element_rect(fill = grDevices::adjustcolor("white", alpha.f = 0.92), colour = "grey60"),
      legend.box.background = element_rect(fill = grDevices::adjustcolor("white", alpha.f = 0.92), colour = "grey60"),
      legend.key.width = grid::unit(7, "pt"),
      legend.key.height = grid::unit(6.5, "pt"),
      legend.text = element_text(size = 4.4),
      legend.margin = margin(2, 2, 2, 2),
      legend.spacing.y = grid::unit(0.5, "pt")
    )

  legend_grob <- gtable::gtable_filter(ggplotGrob(p_with_legend), "guide-box")
  plot_grob <- ggplotGrob(p + theme(legend.position = "none"))
  panel_rows <- grep("^panel", plot_grob$layout$name)
  first_panel <- panel_rows[1L]
  panel_pos <- plot_grob$layout[first_panel, c("t", "l", "b", "r")]
...
make_ks_plot_data <- function(dt, scenario_value, metric) {
  sub <- copy(dt[scenario == scenario_value & type == "KS"])
  if (identical(metric, "power")) sub <- sub[delta >= 0]
...
make_cvm_plot_data <- function(dt, scenario_value, weight_value, metric) {
  sub <- copy(dt[scenario == scenario_value & type == "CvM" & weight_name == weight_value])
  if (identical(metric, "power")) sub <- sub[delta >= 0]
  else sub <- sub[delta > 0]
  out <- aggregate_over_sstar(
    sub,
...
make_alt_plot_data <- function(dt, scenario_value, tuning_value, metric) {
  sub <- copy(dt[scenario == scenario_value & tuning_label_plot == tuning_value])
  if (identical(metric, "power")) sub <- sub[delta >= 0]
  else sub <- sub[delta > 0]
  out <- aggregate_over_sstar(
    sub,
...
densify_plot_paths <- function(dt, group_cols, metric, n = 161L) {
  if (!nrow(dt)) return(list(line_dt = dt, point_dt = dt))
  point_dt <- copy(dt)
  line_dt <- point_dt[
    ,
    {
      ...
      if (length(x) <= 2L) {
        ...
        spline_out <- stats::spline(x = x, y = y, n = max(n, length(x)), method = "natural")
        y_out <- pmin(pmax(spline_out$y, min(y, na.rm = TRUE)), max(y, na.rm = TRUE))
        if (metric %in% c("power", "sap")) {
          y_out <- pmin(pmax(y_out, 0), 1)
        } else {
          y_out <- pmax(y_out, 0)
        }
        .(delta = spline_out$x, value = y_out)
      }
    },
    by = group_cols
  ]
  list(line_dt = line_dt, point_dt = point_dt)
}

save_ks_scenario_plot <- function(dt, scenario_value, metric, filename) {
  plot_dt <- make_ks_plot_data(dt, scenario_value, metric)
  curve_dt <- densify_plot_paths(plot_dt, c("dgp_type", "T", "series"), metric)
  p <- ggplot(
    curve_dt$line_dt,
    aes(x = delta, y = value, group = series, colour = series, linetype = series, shape = series)
  ) +
    geom_line(linewidth = 0.5, lineend = "round", linejoin = "round") +
    geom_point(data = curve_dt$point_dt, size = 0.75, stroke = 0.18) +
...
alt_metric_axis_limits <- function(metric, scenario_value, tuning_value, dgp_value) {
  if (identical(metric, "sap") &&
      identical(scenario_value, "smooth_change") &&
      grepl("^Multiscale MOSUM", tuning_value)) {
    return(coord_cartesian(ylim = c(0, 0.20)))
  }
  if (identical(metric, "sap") &&
      identical(scenario_value, "abrupt_local_change") &&
      grepl("^Multiscale MOSUM", tuning_value)) {
    y_max <- switch(
      dgp_value,
      BB = 0.25,
    ...
    return(coord_cartesian(ylim = c(0, y_max)))
  }
  metric_axis_limits(metric)
}
...
save_ks_dgp_plot <- function(dt, scenario_value, dgp_value, metric, filename,
                             inset_legend = FALSE, inset_args = list()) {
  ...
  p <- ggplot(
    ...
    geom_line(linewidth = 0.52, lineend = "round", linejoin = "round") +
    geom_point(data = curve_dt$point_dt, size = 0.72, stroke = 0.18) +
  ...
  if (isTRUE(inset_legend)) {
    p <- do.call(add_inset_legend_to_first_panel, c(list(p = p), inset_args))
  }
  save_plot_with_nolegend(p, filename, width = 7.2, height = 3.0, nolegend_height = 2.55)
}
...
save_cvm_weight_plot <- function(dt, scenario_value, weight_value, metric, filename) {
  ...
  curve_dt <- densify_plot_paths(plot_dt, c("dgp_type", "T", "standardizer"), metric)
  p <- ggplot(
    curve_dt$line_dt,
...
save_cvm_dgp_plot <- function(dt, scenario_value, weight_value, dgp_value, metric, filename) {
  ...
  p <- ggplot(
    ...
    scale_shape_manual(values = std_shapes, breaks = standardizer_levels) +
    labs(x = expression(Delta), y = metric_label(metric)) +
    guides(colour = guide_legend(nrow = 1), linetype = guide_legend(nrow = 1), shape = guide_legend(nrow = 1)) +
    plot_theme(base_size = 8.2)
  if (!is.null(metric_axis_limits(metric))) p <- p + metric_axis_limits(metric)
  save_plot_with_nolegend(p, filename, width = 7.2, height = 2.9, nolegend_height = 2.45)
}
...
save_alt_dgp_plot <- function(dt, scenario_value, tuning_value, dgp_value, metric, filename) {
  ...
  p <- ggplot(
    ...
    guides(colour = guide_legend(nrow = 1), linetype = guide_legend(nrow = 1), shape = guide_legend(nrow = 1)) +
    plot_theme(base_size = 8.2)
  alt_limits <- alt_metric_axis_limits(metric, scenario_value, tuning_value, dgp_value)
  if (!is.null(alt_limits)) p <- p + alt_limits
  save_plot_with_nolegend(p, filename, width = 7.2, height = 2.9, nolegend_height = 2.45)
}
...
main_null_ks_comment <- function(null_dt) {
  ...
  paste(
    ...
    "The subsequent raw-power comparisons should therefore be interpreted jointly with this null-rejection pattern: HAC attains higher rejection rates partly through greater size distortion, whereas RSMS delivers the stronger power-size compromise within the self-normalized class."
  )
}
...
alt_null_comment <- function(null_dt) {
  paste(
    ...
    "SSMS is again the safest standardizer, while HAC and some RSMS combinations are substantially oversized. The appendix power and ADL comparisons should therefore be interpreted as conditional power-delay comparisons rather than as comparisons among equally calibrated procedures."
  )
}
...
make_alt_metric_comment <- function(power_dt, scenario_value, metric) {
  ...
  if (identical(metric, "adl")) {
    paste(
      ...
      progress,
      sprintf(
        "%s is the slowest retained detector family on average. Consistent with the main KS/CvM evidence, the SSMS rows mainly serve as conservative controls rather than as central supplementary detectors.",
        detector_pretty[[lag_det$detector]]
      ),
      "The retained appendix tables therefore keep the focus on the detector shapes discussed in the supplement."
    )
  } else {
    paste(
      sprintf(
      ...
      "The corresponding SSMS rows remain materially more conservative than their RSMS counterparts, so they are best read as conservative controls rather than as central supplementary detectors.",
      sprintf(
        "%s is the weakest retained detector family on this criterion in the reported designs.",
        detector_pretty[[lag_det$detector]]
      )
    )
  }
...
main_null_path <- file.path(paper_summary_dir, "main_null_summary_alpha_005.csv")
main_power_path <- file.path(paper_summary_dir, "main_power_curve_summary_alpha_005.csv")
alt_null_path <- file.path(paper_summary_dir, "alt_null_summary_alpha_005.csv")
alt_power_path <- file.path(paper_summary_dir, "alt_power_curve_summary_alpha_005.csv")

use_saved_summaries <- FALSE

main_null <- summarize_null_files(null_files, filter_main_dt)
alt_null <- summarize_null_files(null_files, filter_alt_dt)

if (nrow(main_null) == 0L || nrow(alt_null) == 0L) {
  if (all(file.exists(c(main_null_path, main_power_path, alt_null_path, alt_power_path)))) {
    main_null <- fread(main_null_path)
    main_power <- fread(main_power_path)
    alt_null <- fread(alt_null_path)
    alt_power <- fread(alt_power_path)
    use_saved_summaries <- TRUE
  } else {
    stop("No null summaries were produced, and no saved paper-summary CSVs were found.")
  }
}
...
if (!use_saved_summaries) {
  ...
  if (nrow(main_power) == 0L || nrow(alt_power) == 0L) {
    if (all(file.exists(c(main_null_path, main_power_path, alt_null_path, alt_power_path)))) {
      main_null <- fread(main_null_path)
      main_power <- fread(main_power_path)
      alt_null <- fread(alt_null_path)
      ...
      stop("No power summaries were produced, and no saved paper-summary CSVs were found.")
    }
  } else {
    ...
    fwrite(main_null, main_null_path)
    fwrite(main_power, main_power_path)
    fwrite(alt_null, alt_null_path)
    fwrite(alt_power, alt_power_path)
  }
}
...
for (scenario_value in all_scenarios) {
  appendix_block <- c(
    ...
    sprintf("\\subsubsection{%s}", scenario_pretty[[scenario_value]]),
    "",
    sprintf("Under the %s setting, the appendix tables retain the DGP, the horizon $T$, and the positive break grid for the FPCA-compressed weighted-CUSUM, Page-CUSUM, and multiscale MOSUM detectors. The accompanying color figures are again generated DGP by DGP, with the four internal columns corresponding to $T=1,2,5,10$.", scenario_pretty_adj[[scenario_value]]),
    ""
  )
  for (metric in metrics) {
    appendix_block <- c(
