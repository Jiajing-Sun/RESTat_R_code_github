# ==============================================================
# simulation_core.R -- one-replicate simulation engine (Windows-safe)
# ============================================================== 
...
make_method_error_rows <- function(method_row, alpha_levels, rep_id, sim_par, err_msg) {
  ...
  for (i in seq_along(alpha_levels)) {
    ...
    out[[i]] <- data.frame(
      ...
      train_contam = sim_par$train_contam %||% NA_character_,
      train_b = sim_par$train_b %||% NA_real_,
      train_break_frac = sim_par$train_break_frac %||% NA_real_,
      train_drift_start_frac = sim_par$train_drift_start_frac %||% NA_real_,
      contam_shape = sim_par$contam_shape %||% NA_character_,
      t_train_star = sim_par$t_train_star %||% NA_integer_,
...
simulate_one_replicate <- function(rep_id,
  ...
  tryCatch({
    ...
    base <- generate_base_curves(
      dgp_type = sim_par$dgp_type,
      total_n = total_n,
      n_grid = sim_par$n_grid,
      basis_type = sim_par$basis_type %||% "bspline",
      nbasis = sim_par$nbasis %||% 21L,
      muInfo = sim_par$muInfo %||% list(type = "sin", a = 1),
      varType = sim_par$varType %||% "A",
      gaussian = if (is.null(sim_par$gaussian)) TRUE else isTRUE(sim_par$gaussian),
      kappa = sim_par$kappa %||% 0.7
    )

    curve_values <- apply_change_to_curves(
      curves = base$values,
      t_grid = base$t_grid,
      basis_matrix = base$basis_matrix,
      scenario = sim_par$scenario,
      m = sim_par$m,
      T = sim_par$T_value,
      delta = sim_par$delta,
    ...
    contam_info <- apply_training_contamination_to_curves(
      curves = curve_values,
      t_grid = base$t_grid,
      basis_matrix = base$basis_matrix,
      m = sim_par$m,
      contam_type = sim_par$train_contam %||% "none",
      train_b = sim_par$train_b %||% 0,
      train_break_frac = sim_par$train_break_frac %||% 0.8,
      train_drift_start_frac = sim_par$train_drift_start_frac %||% 0.0,
      contam_shape = sim_par$contam_shape %||% "phase2_matched",
      phase2_scenario = sim_par$scenario,
      basis_k = sim_par$basis_k
    )
    curve_values <- contam_info$curves
    sim_par$t_train_star <- contam_info$t_train_star

    fpca <- fpca_project_scores(curve_values,
                                m = sim_par$m,
                                q_cap = sim_par$q_cap,
                                fve_threshold = sim_par$fve_threshold,
    ...
    out <- fill_or_create_column(out, "train_contam", sim_par$train_contam %||% "none")
    out <- fill_or_create_column(out, "train_b", sim_par$train_b %||% 0)
    out <- fill_or_create_column(out, "train_break_frac", sim_par$train_break_frac %||% NA_real_)
    out <- fill_or_create_column(out, "train_drift_start_frac", sim_par$train_drift_start_frac %||% NA_real_)
    out <- fill_or_create_column(out, "contam_shape", sim_par$contam_shape %||% "phase2_matched")
    out <- fill_or_create_column(out, "t_train_star", sim_par$t_train_star %||% NA_integer_)
    out
  }, error = function(e) {
