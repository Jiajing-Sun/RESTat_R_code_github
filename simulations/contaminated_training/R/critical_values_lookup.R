...
first_existing_path <- function(paths, label) {
  hit <- unique(paths)[file.exists(unique(paths))]
  if (length(hit) > 0L) return(hit[1L])
  stop(
    "Missing ", label, ". Tried:\n",
    paste(unique(paths), collapse = "\n"),
    "\nGenerate critical values with the scripts under ../../critical_values/."
  )
}
...
candidate_main_cv_paths <- function(root) {
  ...
  list(
    base = unique(c(
      ...
      file.path(dirname(dirname(root)), "critical_values", "main", "outputs", "critical_values_all.csv")
    )),
    weights = unique(c(
      ...
      file.path(dirname(dirname(root)), "critical_values", "main", "outputs", "critical_values_all_weights.csv")
    ))
  )
}
...
candidate_alt_cv_paths <- function(root) {
  ...
  unique(c(
    ...
    file.path(dirname(dirname(root)), "critical_values", "alternative_detectors", "outputs", "critical_values_alt_detectors.csv")
  ))
}

load_main_critical_values <- function(root) {
  paths <- candidate_main_cv_paths(root)
  base_path <- first_existing_path(paths$base, "main critical value file")
  weight_path <- first_existing_path(paths$weights, "weighted critical value file")
...
load_alt_critical_values <- function(root) {
  path <- first_existing_path(candidate_alt_cv_paths(root), "alternative-detector critical value file")
