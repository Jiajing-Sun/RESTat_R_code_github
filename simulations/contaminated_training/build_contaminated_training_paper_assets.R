...
# Build paper tables, figures, and notes for the
...
style_for_method <- function(label) {
  if (grepl("^RSMS", label) && grepl("gamma=0.15", label)) return(list(col = method_colors[["RSMS"]], lty = 2, lwd = 2.2, pch = 1))
  if (grepl("^RSMS", label)) return(list(col = method_colors[["RSMS"]], lty = 1, lwd = 2.2, pch = 16))
  if (grepl("^SSMS", label) && grepl("gamma=0.15", label)) return(list(col = method_colors[["SSMS"]], lty = 2, lwd = 2.0, pch = 0))
  if (grepl("^SSMS", label)) return(list(col = method_colors[["SSMS"]], lty = 1, lwd = 2.0, pch = 15))
  if (grepl("^HAC", label) && grepl("gamma=0.15", label)) return(list(col = method_colors[["HAC"]], lty = 2, lwd = 1.9, pch = 2))
  if (grepl("^HAC", label)) return(list(col = method_colors[["HAC"]], lty = 1, lwd = 1.9, pch = 17))
  list(col = "black", lty = 1, lwd = 2, pch = 16)
}

style_for_multiscale <- function(label) {
  if (grepl("^RSMS", label)) return(list(col = method_colors[["RSMS"]], lty = 1, lwd = 2.3, pch = 16))
  if (grepl("^SSMS", label)) return(list(col = method_colors[["SSMS"]], lty = 1, lwd = 2.0, pch = 15))
  if (grepl("^HAC", label)) return(list(col = method_colors[["HAC"]], lty = 1, lwd = 1.9, pch = 17))
  list(col = "black", lty = 1, lwd = 2, pch = 16)
}
...
smooth_curve_xy <- function(x, y, n_out = 600L) {
  ok <- is.finite(x) & is.finite(y)
  x <- x[ok]
  y <- y[ok]
  if (length(x) < 4L) return(list(x = x, y = y))
  use_hyman <- all(diff(y) >= 0) || all(diff(y) <= 0)
  sm <- stats::spline(
    x = x,
    y = y,
    n = max(n_out, 50L * length(x)),
    method = if (use_hyman) "hyman" else "natural"
  )
  sm$y <- pmin(1, pmax(0, sm$y))
  sm
}
...
plot_metric_family <- function(df, dgp_value, method_order, metric, outfile, title_text, subtitle_text = NULL,
                               style_fun = style_for_method, add_legend = FALSE,
                               panel_layout = c("row", "column")) {
  panel_layout <- match.arg(panel_layout)
  contam_levels <- order_contam(df$train_contam)
  b_vals <- order_b_train(df$train_b)
  bottom_oma <- if (isTRUE(add_legend)) 4.8 else 0
  if (identical(panel_layout, "column")) {
    png(outfile, width = 1300, height = if (isTRUE(add_legend)) 1650 else 1500, res = 170)
    par(mfrow = c(length(contam_levels), 1), mar = c(4.8, 4.8, 3.1, 1.1), oma = c(bottom_oma, 0, 5.2, 0), xpd = NA)
  } else {
    png(outfile, width = 2000, height = if (isTRUE(add_legend)) 1120 else 980, res = 170)
    par(mfrow = c(1, length(contam_levels)), mar = c(4.8, 4.6, 3.1, 1.1), oma = c(bottom_oma, 0, 5.2, 0), xpd = NA)
  }
  ylim <- c(0, 1)
  for (ii in seq_along(contam_levels)) {
    ct <- contam_levels[ii]
    ...
    for (lab in method_order) {
      ...
      sty <- style_fun(lab)
      sm <- smooth_curve_xy(dd$train_b, dd[[metric]])
      lines(sm$x, sm$y, col = sty$col, lty = sty$lty, lwd = sty$lwd)
  ...
  if (isTRUE(add_legend)) {
    par(fig = c(0, 1, 0, 0.16), mar = c(0, 0, 0, 0), new = TRUE, xpd = NA)
    plot.new()
    legend("center", bty = "n", cex = 0.72,
           ...
           ncol = min(3L, length(method_order)), pt.cex = 0.9, seg.len = 2.6,
           x.intersp = 0.8, y.intersp = 0.9)
  }
  dev.off()
}

nolegend_path <- function(path) {
  sub("\\.png$", "_nolegend.png", path)
}

plot_metric_family_versions <- function(df, dgp_value, method_order, metric, outfile, title_text,
                                        subtitle_text = NULL, style_fun = style_for_method,
                                        panel_layout = c("row", "column")) {
  panel_layout <- match.arg(panel_layout)
  plot_metric_family(df, dgp_value, method_order, metric, nolegend_path(outfile), title_text,
                     subtitle_text = subtitle_text, style_fun = style_fun,
                     add_legend = FALSE, panel_layout = panel_layout)
  plot_metric_family(df, dgp_value, method_order, metric, outfile, title_text,
                     subtitle_text = subtitle_text, style_fun = style_fun,
                     add_legend = TRUE, panel_layout = panel_layout)
}
...
plot_metric_family_versions(paper_df, paper_dgp, main_ks_order, "post_break_detect_rate", main_ks_postbreak_fig,
                            title_text = "Post-break detection probability", subtitle_text = "Main KS robustness",
                            panel_layout = "row")
plot_metric_family_versions(paper_df, paper_dgp, main_ks_order, "pre_break_stop_rate", main_ks_prebreak_fig,
                            title_text = "Pre-break stopping probability", subtitle_text = "Main KS robustness",
                            panel_layout = "row")
plot_metric_family_versions(paper_df, paper_dgp, alt_cusum_order[1:6], "post_break_detect_rate", alt_weighted_postbreak_fig,
                            title_text = "Post-break detection probability", subtitle_text = "Weighted CUSUM robustness",
                            panel_layout = "row")
plot_metric_family_versions(paper_df, paper_dgp, alt_cusum_order[1:6], "pre_break_stop_rate", alt_weighted_prebreak_fig,
                            title_text = "Pre-break stopping probability", subtitle_text = "Weighted CUSUM robustness",
                            panel_layout = "row")
plot_metric_family_versions(paper_df, paper_dgp, alt_multiscale_order, "post_break_detect_rate", alt_multiscale_postbreak_fig,
                            title_text = "Post-break detection probability", subtitle_text = "Multiscale MOSUM robustness",
                            style_fun = style_for_multiscale, panel_layout = "row")
plot_metric_family_versions(paper_df, paper_dgp, alt_multiscale_order, "pre_break_stop_rate", alt_multiscale_prebreak_fig,
...
main_ks_table_tex <- make_two_block_table(
  ...
  caption = "Effect of mild training contamination on post-break detection for the main KS statistics under the fMA(1) setting. Configuration: level-shift setting, $m=1000$, $T=2$, $s^{\\star}/(mT)=0.8$, nominal level $\\alpha=0.05$, and Monte Carlo critical values from clean training samples. Entries report $100\\times P(s^{\\star}<\\tau_m\\le mT)$.",
...
main_cvm_table_tex <- make_weighted_cvm_table(
  paper_df,
  metric = "post_break_detect_rate",
  caption = "Effect of mild training contamination on post-break detection for the weighted-CvM statistics under the fMA(1) setting. Configuration: level-shift setting, $m=1000$, $T=2$, $s^{\\star}/(mT)=0.8$, nominal level $\\alpha=0.05$, and critical values from clean training samples.",
...
alt_cusum_table_tex <- make_two_block_table(
  ...
  metric = "post_break_detect_rate",
  caption = "Effect of mild training contamination on post-break detection for the CUSUM-style supplementary statistics under the fMA(1) setting. Configuration: level-shift setting, $m=1000$, $T=2$, $s^{\\star}/(mT)=0.8$, and $\\alpha=0.05$.",
...
alt_multiscale_table_tex <- make_two_block_table(
  ...
  metric = "post_break_detect_rate",
  caption = "Effect of mild training contamination on post-break detection for the multiscale MOSUM supplementary statistic under the fMA(1) setting. Configuration: level-shift setting, $m=1000$, $T=2$, $s^{\\star}/(mT)=0.8$, and $\\alpha=0.05$.",
...
notes <- c(
  ...
  sprintf("The selected figures and tables are written for the paper; matching CSV summaries remain in outputs/%s/summary.", output_tier)
)
atomic_write_lines(notes, file.path(asset_dir, "contam_paper_notes.txt"))

message("Wrote contamination tables and figures to: ", normalize_path2(asset_dir, mustWork = FALSE))
