...
find_project_root <- function(start = NULL,
                              markers = c("run_generate_streamingcurve_cv.R", file.path("R", "project_paths.R"))) {
  ...
  stop(
    "Could not locate the project root. Run the script from inside the project folder, ",
    "or pass --root=/path/to/streaming_curve_cv_codes."
  )
}

resolve_project_root <- function(default_start = NULL) {
  args <- parse_named_args()
  env_root <- Sys.getenv("STREAMING_CURVE_CV_ROOT", unset = "")
