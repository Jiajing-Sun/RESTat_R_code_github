...
safe_matrix <- function(x, ncol = NULL) {
  ...
  if (is.null(ncol)) return(matrix(x, ncol = 1L))
  matrix(x, ncol = ncol)
}
...
write_csv_atomic <- function(x, path, row.names = FALSE, ...) {
  ...
  if (!ok) stop("Could not move temp CSV into place: ", path)
  invisible(path)
}
...
method_label_alt <- function(standardizer, detector, gamma = NA_real_, bandwidth_h = NA_real_,
                             omega_name = NA_character_, hset_name = NA_character_,
