...
get_critical_value_rsms <- function(T.chan, alpha = 0.05, gamma = 0, d) {
  ...
  offset <- ifelse(gamma == 0, 0, 8)  # gamma = 0.15 is offset by 8 columns
  
  # Critical value matrix: rows = d (1:8), columns = 8 for gamma=0, 8 for gamma=0.15
  crit_mat <- matrix(c(
    # gamma=0 (columns 1-8)
...
get_critical_value_ssms <- function(T.chan, alpha = 0.05, gamma = 0, d) {
  ...
  # Critical value matrix: rows = d (1:10), columns = 8 for gamma=0, 8 for gamma=0.15
  crit_mat <- matrix(c(
    # gamma=0 (columns 1-8)
...
get_critical_value_csms <- function(T.chan, alpha = 0.05, gamma = 0, d) {
  ...
  col_idx <- col_map[[alpha_key]][[t_key]]
  offset <- ifelse(gamma == 0, 0, 8)  # gamma=0.15 offset by 8 columns
  
  # Critical value matrix: rows = d (1:8), columns = 8 for gamma=0, 8 for gamma=0.15
  crit_mat <- matrix(c(
    # gamma = 0 (columns 1--8): (T=1: 5%,10%), (T=2: 5%,10%), (T=5: 5%,10%), (T=10: 5%,10%)
    ...
    # gamma = 0.15 (columns 9--16) in the same order of (T, alpha)
